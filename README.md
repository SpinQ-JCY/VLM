...
system   = "你是一个乐于助人的助手，可以根据图片内容回答问题。"
im_end   = tokenizer.eos_token   # Qwen3-1.7B 为 <|im_end|>

训练整段文本（build_train_text，truncate 至 128 token，add_special_tokens=False）：
  <|im_start|>system
  你是一个乐于助人的助手，可以根据图片内容回答问题。
  <|im_end|>
  <|im_start|>user
  <image>
  请用一句话描述这张图片。
...
② 文本 tokenize（T_text=46，含 1 个 <image> 占位 id=151669）
  input_ids             (46,)
...
③ 在 pos=22 处展开 <image>
  ...
  suffix  text[23:]      (23, 2048)    问题 + <|im_end|> + assistant 头 + answer + <|im_end|>
  ────────────────────────────────────
  merged  cat 沿 seq 维  (621, 2048)   621 = 46 - 1 + 576

④ 送入 Qwen3 LLM
  inputs_embeds         (1, 621, 2048)
  labels                (1, 621)       前 611 为 -100；后 10 个为 answer + <|im_end|> 算 loss
    → Causal LM         (1, 621, 151670)

推理（仅 prompt）时 T_text=36 → T_seq=611，generate 再追加新 token。
```

验证：`python models/vlms/VLM_v1_model.py`
...
InstructFT + LoRA（Step 5b 权重；`--lora-dir` 可省略，会按 projector 自动匹配 `lora/` 或 `lora_step_{N}/`）：
...
| 8.2 | VLM 逐条回答（每题单独推理）    | eval/outputs/step8_2_answers_{权重名}.json          |
...
InstructFT + LoRA（Step 5b）复用同一份 8.0 / 8.1 基准，只需单独跑 8.2 / 8.3。`--lora-dir` 可省略，会按 projector 自动匹配成对的 `lora_step_{N}/`：

```bash
# 停 vLLM 后 8.2：LoRA 逐条回答
python eval/step8_2_vlm_answer.py \
  --checkpoint checkpoints/instructft_lora/projector_step_5000.pt

# 重启 vLLM 后 8.3：LoRA 打分
python eval/step8_3_judge_scores.py \
  --input eval/outputs/step8_2_answers_instructft_lora_step_5000.json \
  --force
```

输出示例：`step8_2_answers_instructft_lora_step_5000.json` → `step8_3_scores_instructft_lora_step_5000.json`（与纯 InstructFT 结果分开，便于对比）。
