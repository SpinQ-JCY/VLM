...
scene 类型汇总为 Align 语义对齐得分；detail 类型汇总为 InstructFT 指令微调得分。
"""
...
SCENE_RUBRIC = f"""【题型】scene — 描述图片主要内容（测评 Align 语义对齐）
...
DETAIL_RUBRIC = f"""【题型】detail — 回答针对图片细节的具体问题（测评 InstructFT 指令微调）
...
def summarize(items: list[dict]) -> dict:
    ...
    return {
        ...
        "semantic_align_scene": scene,
        "instructft_detail": detail,
        "overall": {
...
def main() -> None:
    ...
    parser.add_argument("--input", type=Path, default=None, help="Step 8.2 输出（默认 InstructFT 权重对应文件）")
...
def _print_summary(summary: dict) -> None:
    ...
    _print_block("Align 语义对齐 (scene)", summary["semantic_align_scene"])
    _print_block("InstructFT 指令微调 (detail)", summary["instructft_detail"])
    o = summary["overall"]
    if o.get("count"):
        print(
            f"\noverall: n={o['count']}  "
